...
ARG CUDA_RUNFILE=cuda_12.6.0_560.28.03_linux.run
...
# Copy your local cuDNN .deb file into the container
COPY cudnn-local-repo-debian11-8.9.7.29_1.0-1_amd64.deb /tmp/

RUN dpkg -i /tmp/cudnn-local-repo-debian11-8.9.7.29_1.0-1_amd64.deb && \
    apt-get update --allow-insecure-repositories && \
    apt-get install -y --allow-unauthenticated libcudnn8 libcudnn8-dev && \
    rm /tmp/cudnn-local-repo-debian11-8.9.7.29_1.0-1_amd64.deb && \
    rm -rf /var/lib/apt/lists/*
